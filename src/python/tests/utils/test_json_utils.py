...
def test_get_json_loads_data(tmp_path: Path) -> None:
    """Test that `json_utils.get_json()` loads JSON from a file and returns the parsed object."""
    p = tmp_path / "in.json"
...
def test_get_json_passes_parse_int_kwarg(tmp_path: Path) -> None:
    """Test that `json_utils.get_json()` forwards `parse_int` to the JSON parser."""
...
def test_print_json_uses_defaults_sort_keys_and_indent(tmp_path: Path) -> None:
    """Test that `json_utils.print_json()` formats JSON using default sorting and indentation."""
    p = tmp_path / "out.json"
    ...
    assert json.loads(json_text) == {"a": 1, "b": 2}


def test_print_json_respects_overrides_for_sort_keys_and_indent(tmp_path: Path) -> None:
    """Test that `json_utils.print_json()` respects explicit sort key and indentation options."""
    p = tmp_path / "out.json"
    ...
    assert "\n  " in json_text
    assert "\n    " not in json_text
    assert json_text.index('"b"') < json_text.index('"a"')
    assert json.loads(json_text) == {"a": 1, "b": 2}


def test_print_json_allows_non_default_serialization_options(tmp_path: Path) -> None:
    """Test that `json_utils.print_json()` can write JSON using custom serialization options."""
    p = tmp_path / "out.json"

...
def test_make_seqregion_from_gbff(
    ...
    Args:
        genbank_id: Expected Genbank ID.
        codon_table: Codon table to use.
        location: Organelle location.
        is_circular: Boolean indicating if sequence region is circular.
        expected_seq: Expected dictionary describing sequence region.
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            ...
            "<html></html>",
            None,
            pytest.raises(ValueError, match=r"^Response from .* is not JSON$"),
...
def test_add_mitochondrial_codon_table(
    ...
    Args:
        ...
        mock_response: Factory returning a mocked HTTP response object.
        input_seq: Sequence dict with usable values (`codon_table`, `location`).

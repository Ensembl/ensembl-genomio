...
class Manifest:
    ...
    def get_files_checksums(self) -> ManifestDict:
        ...
        for subfile in self.root_dir.iterdir():
            ...
            for name, standard_name in _NAMES.items():
                ...
                if subfile.stem.endswith(name) or subfile.suffix == f".{name}":
                    ...
                    if standard_name in _MULTI_FILES:
    ...
    def _prepare_object_name(
        ...
        subfile: Path,
        name: str,
        manifest_file_dict: dict[str, dict[str, str]],
    ) -> str:
        # Prepare object name
        try:

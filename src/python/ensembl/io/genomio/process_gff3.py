...
class InputSchema(argschema.ArgSchema):
    ...
    in_gff_path = argschema.fields.InputFile(metadata={
        "required": True, "description": "gene.gff file required"
    })
    genome_data = argschema.fields.InputFile(metadata={
        "required": True, "description": "genome.json file required"
    })
...
class process_gff3():
    ...
    def normalize_gene(self, gene: SeqFeature, functional_annotation: List, fail_types: List) -> SeqFeature:
        ...
        for count, transcript in enumerate(gene.sub_features):
            if transcript.type not in allowed_transcript_types and transcript.type not in self.ignored_transcript_types:

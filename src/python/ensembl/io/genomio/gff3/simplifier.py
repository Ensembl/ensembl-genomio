...
class GFFSimplifier:
    ...
    def simpler_gff3(self, in_gff_path: PathLike) -> None:
        """Load a GFF3 from INSDC and rewrites it in a simpler version.

        Also writes a functional annotation file.
        """
    ...
    def normalize_transcripts(self, gene: GFFSeqFeature) -> None:
        ...
        for count, transcript in enumerate(gene.sub_features):
            ...
            self.format_gene_segments(transcript)

            # EXONS AND CDS
            self._normalize_transcript_subfeatures(gene, transcript)
    ...
    def normalize_mirna(self, gene: GFFSeqFeature) -> list[GFFSeqFeature]:
        ...
        for new_gene in all_genes:
            normalized_gene = self.normalize_gene(new_gene)
            self.annotations.store_gene(normalized_gene)
            all_genes_cleaned.append(self.clean_gene(normalized_gene))

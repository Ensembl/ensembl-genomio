...
def check_genome(genome, extra):
    ...
    if not operations:
        ...
        if "Load from INSDC" in operations:
            source_count += 1
        if "Load from RefSeq" in operations:
            source_count += 1
        if "Load from EnsEMBL" in operations:
            source_count += 1
            ...
            if new_abbrev in all_abbrevs:
                if "Replacement" in extra:
                    ...
                    failed_issues.append(
                        {"issue": issue, "desc": f"Abbrev {new_abbrev} used by other species"}
                    )
            elif cur_abbrev:
                if cur_abbrev == new_abbrev:
                    if "Replacement" in extra and not new_abbrev in all_abbrevs:
                        warnings_replace.append(
                            {"issue": issue, "desc": f"replacement genome has new abbrev: {new_abbrev}"}
                        )
                    else:
                        ok_exist.append({"issue": issue, "desc": new_abbrev})
                else:
                    warnings.append(
                        {"issue": issue, "desc": f"Generated id differs: {cur_abbrev} vs {new_abbrev}"}
                    )
...
def add_organism_to_issue(redmine, issue, new_abbrev, update=False):
    ...
    if update:
        ...
        if "Organism Abbreviation" in custom:
            ...
            feedback = redmine.issue.update(issue.id, custom_fields=[{"id": field_id, "value": new_abbrev}])
            if not feedback:
...
def make_organism_abbrev(name):
    ...
    genus = re.sub("[\[\]]", "", genus)
    strain_abbrev = re.sub(
        r"(isolate|strain|breed|str\.|subspecies|sp\.)", "", strain_abbrev, flags=re.IGNORECASE
    )
...
def main():
    ...
    parser.add_argument(
        "--get",
        choices=["genomes", "organism_abbrev", "abbreviate"],
        required=True,
    ...
    parser.add_argument(
        "--current_abbrevs", type=str, help="File that contains the list of current organism_abbrevs"
    )
    parser.add_argument(
        "--update_redmine",
        action="store_true",
    ...
    parser.add_argument("--organism", type=str, help="Organism name to abbreviate")

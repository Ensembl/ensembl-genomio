...
def get_intervals( record, genesDict, seqDict, seqname ):
    ...
    for r in record.features :
        ...
        genesDict[ str(r.id) ] = { 'sequence' : record.id , 'start' : int(r.location.start) + 1, 'end' : int(r.location.end), 'strand' : r.location.strand, 'name' : r.id }
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Scan a GFF file to detect overlapping seqfeature objects (default = gene).')
    parser.add_argument('--gff', help='path of gff file to scan for overlaps', required=True, nargs='?' )
    parser.add_argument('--stats', help='run stats analysis of GFF file instead of parsing GFF file', action='store_true' )
    parser.add_argument('--logfile', help='path of file for logging (default=log.overlaps)', nargs='?', default='log.overlaps' )
    parser.add_argument('--output', help='path of file for output (default=overlaps)', required=True, nargs='?', default='overlaps' )
    parser.add_argument('--filter', help='seqfeature type to use for overlap isolation (default=gene)', nargs='?', default='gene' )

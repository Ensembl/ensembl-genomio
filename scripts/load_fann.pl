...
&GetOptions(
  ...
  'skip_ensembl_xref:i'        => \$skip_ensembl_xref,
  'help|?'                     => \$help,
...
for my $it (@$data) {
  ...
  @xrefs = remove_ensembl_xrefs(@xrefs) if $skip_ensembl_xref;
...
sub remove_ensembl_xrefs {
  ...
  for my $xref (@xrefs) {
    my $dbname = $xref->{dbname};
    $dbname = $extdb_map->{$dbname} if exists $extdb_map->{$dbname};
    if (not exists $ensembl_xrefs{ $dbname }) {
      push @new_xrefs, $xref;
    }
  }
